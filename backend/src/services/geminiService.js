...
async function checkDuplicates(description, issueType, nearbyComplaints) {
    const m = getModel();
    if (!m || nearbyComplaints.length === 0) return [];

    const complaintsText = nearbyComplaints.map((c, i) =>
        `[${i}] ID: ${c.complaint_id}, Type: ${c.issue_type}, Status: ${c.status}, Area: ${c.area}, Description: ${c.description}`
    ).join('\n');

    const prompt = `A citizen is about to report a new issue:
Type: ${issueType}
Description: "${description}"

Here are existing open complaints nearby:
${complaintsText}

Which of these existing complaints, if any, describe the SAME issue? Return a JSON array of matches. Each match should have:
- index: the complaint index number
...
Respond with ONLY the JSON array, no markdown.`;
    ...
    try {
        ...
        return matches.map(m => ({
            complaintId: nearbyComplaints[m.index]?.complaint_id,
            complaint: nearbyComplaints[m.index],
            similarity: m.similarity,
            reason: m.reason
        })).filter(m => m.complaintId);
    } catch (_) {
        return [];
    }
}

module.exports = { chat, analyzeDescription, enhanceDescription, checkDuplicates };
